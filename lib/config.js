...
import { deepClone, isUrl, ensureHttps } from './utils'
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  switch (key) {
    ...
    case 'UUID_REDIRECT':
      ...
      const result = convertVal(
        getValue(extendConfig[key], getValue(defaultVal, BLOG[key]))
      )
      
      // 对于URL类型的配置，确保使用HTTPS协议
      if (key === 'LINK' || key === 'CONTACT_GITHUB' || key === 'CONTACT_TWITTER' || 
          key === 'CONTACT_FACEBOOK' || key === 'CONTACT_INSTAGRAM' || 
          key === 'CONTACT_LINKEDIN' || key === 'CONTACT_YOUTUBE') {
