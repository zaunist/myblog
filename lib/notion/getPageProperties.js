...
export default async function getPageProperties(
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      // 处理 lm 类型数据
      if (val && Array.isArray(val) && val[0] === 'lm') {
        properties[schema[key].name] = val[1]?.href || ''
      } else {
  ...
  properties.pageCoverThumbnail = mapImgUrl(value?.format?.page_cover, value, 'block') ?? null
